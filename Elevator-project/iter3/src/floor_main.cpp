...
// Helper to send a UDP floor request to the Scheduler.
...
int main() {
    ...
    // Create a Floor object (pass nullptr because we now use UDP).
    Floor floor(nullptr);
    ...
    // Bind to FLOOR_PORT to receive elevator status updates.
    ...
    // Thread to read from input.txt, parse each line, and send a floor request.
    std::thread requestThread([&](){
        ...
        while (std::getline(inputFile, line)) {
            ...
            std::string timestamp, floorStr, directionStr;
            std::getline(ss, timestamp, ',');
            std::getline(ss, floorStr, ',');
            std::getline(ss, directionStr, ',');
            // Lambda to trim whitespace from a string.
            ...
            // Build the message using the same format as before.
            std::string reqMsg = "FLOOR_REQUEST," + timestamp + "," + floorStr + "," + (dir == UP ? "UP" : "DOWN");
    ...
    // Thread to listen for elevator status updates from the Scheduler.
